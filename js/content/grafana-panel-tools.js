...
(() => {
    ...
    const cpuCapacityLegend = window.DashBridgeGrafanaCpuCapacityLegend?.create({
        tools, visualMetadata, getLegendLabel, syncThresholdHighlightState
    });
    if (!cpuCapacityLegend) throw new Error('DashBridge Grafana vCPU legend runtime is unavailable');
    registerRuntimeCleanup(cpuCapacityLegend.stop);
    const attachCpuCapacityToReportSnapshot = cpuCapacityLegend.attachToReportSnapshot;
    const syncCpuCapacityLegend = cpuCapacityLegend.sync;
