# Документация DashBridge

В этом каталоге остаются только документы, которые нужны для поддержки
текущего расширения, и короткая история архитектурно значимых решений.

## Актуальные документы

- [`../AGENTS.md`](../AGENTS.md) — обязательный порядок чтения и правила
  AI-анализа/ревью репозитория.
- [`../README.md`](../README.md) — пользовательские функции, установка и
  запуск проверок.
- [`architecture.md`](architecture.md) — источник
  истины по архитектуре, runtime-потокам и пользовательским сценариям.
- [`development-guide.md`](development-guide.md) — инварианты разработки,
  зоны риска и обязательные проверки.
- [`module-design.md`](module-design.md) — правила создания, разделения и
  объединения модулей, lifecycle-контракт и проверяемые пределы размера.
- [`permission-map.md`](permission-map.md) — связь разрешений Chrome с
  функциями расширения.
- [`dashflow-v2.md`](dashflow-v2.md) — формат записи Traffic Recorder,
  полнота данных и границы конвертации в HAR/JMX/WebPageTest/SAZ.
- [`installer.md`](installer.md) — установка и обновление unpacked DashBridge,
  поиск существующей папки, staging/backup и release trust boundary.
- [`roadmap.md`](roadmap.md) — только актуальные незавершённые
  направления. Реализованные задачи там не хранятся.

## История

- [`history/architecture-decisions.md`](history/architecture-decisions.md) —
  сводка решений, полученных из прежних аудитов и implementation-планов.

Старые пошаговые планы и design-черновики удалены: они описывали промежуточную
структуру файлов, содержали устаревшие номера строк и незакрытые чекбоксы после
реализации. История изменений не является источником текущего поведения — его
определяют код, тесты и `architecture.md`; `AGENTS.md` заставляет
AI-агента начать анализ с этой карты, но не подменяет проверку реализации.

Статический полный инвентарь строк и байтов также не хранится: он устаревает
после каждой правки. Актуальные production-границы и исключения проверяют
`scripts/check-module-boundaries.js` и `scripts/module-size-budgets.json`, а
устойчивые причины разделения или объединения сохраняются в архитектуре и
истории решений. Неподключённые code-прототипы не являются документацией;
незавершённая продуктовая идея сначала фиксируется в `roadmap.md` без копии
предполагаемой production-реализации.

## Корень репозитория

В корне намеренно остаются только файлы, которые инструменты или пользователь
ожидают найти сразу: `.gitignore`, `AGENTS.md`, `README.md`, `manifest.json`,
`package.json`, lock-файл и `eslint.config.js`. Архитектура, правила разработки,
форматы и история принадлежат `docs/`.

`dist/`, `node_modules/` и `test-results/` могут существовать локально, но
игнорируются Git и release whitelist: это воспроизводимые артефакты, а не часть
исходного расширения. Не добавляй в корень временные планы, копии проекта,
diagnostics или выгрузки Traffic Recorder.
