(function initDashBridgeReportAudit(root) {
    ...
    function create({ reportEngine, collect }) {
        if (!reportEngine?.renderPanel || !Array.isArray(reportEngine.PROFILE_VARIABLES)
            || typeof collect !== 'function') throw new TypeError('Report audit requires report engine and collector');
        ...
        const render = (host, result) => {
            host.textContent = '';
            const summary = appendText(host, 'section', 'report-audit-summary', '');
            appendText(summary, 'strong', result.summary.errors ? 'report-audit-fail' : 'report-audit-pass',
                result.summary.errors ? `Ошибок: ${result.summary.errors}` : 'Ошибок нет');
            appendText(summary, 'span', '', `Предупреждений: ${result.summary.warnings}`);
            appendText(summary, 'span', '', `Переменных используется: ${result.summary.usedVariables} из ${result.summary.variables}`);
            appendText(summary, 'span', '', `Панелей проверено: ${result.summary.panels}`);
            appendText(summary, 'span', '', `Контракт движка: ${result.selfCheck.checked} переменных`);

            const issueSection = appendText(host, 'section', 'report-audit-section', '');
            appendText(issueSection, 'h4', '', 'Замечания');
            if (!result.issues.length) appendText(issueSection, 'p', 'report-audit-empty', 'Замечаний нет.');
            else {
                const list = document.createElement('ul'); list.className = 'report-audit-issues'; issueSection.appendChild(list);
                result.issues.forEach(issue => appendText(list, 'li', `report-audit-${issue.level}`, issue.message));
            }

            const variableSection = appendText(host, 'section', 'report-audit-section', '');
            appendText(variableSection, 'h4', '', 'Переменные');
            const tableWrap = appendText(variableSection, 'div', 'report-audit-table-wrap', '');
            const table = document.createElement('table'); table.className = 'report-audit-table'; tableWrap.appendChild(table);
            const head = document.createElement('thead'); const headRow = document.createElement('tr'); head.appendChild(headRow);
            ['Переменная', 'Область', 'В шаблонах', 'Живые данные', 'Где используется'].forEach(value => appendText(headRow, 'th', '', value));
            table.appendChild(head);
            const body = document.createElement('tbody'); table.appendChild(body);
            const scopeLabels = { profile: 'Общий шаблон', panel: 'Панель', list: 'Строка списка' };
            result.variables.forEach(variable => {
                const row = document.createElement('tr'); body.appendChild(row);
                appendText(row, 'td', 'report-audit-variable', `{{${variable.name}}}`);
                appendText(row, 'td', '', scopeLabels[variable.scope]);
                appendText(row, 'td', variable.used ? 'report-audit-pass' : 'report-audit-muted', variable.used ? 'Да' : 'Нет');
                appendText(row, 'td', variable.hasData ? 'report-audit-pass' : 'report-audit-muted', variable.hasData ? (variable.value || 'Есть') : 'Пусто');
                appendText(row, 'td', 'report-audit-locations', variable.locations.join('; ') || '—');
            });

            const panelSection = appendText(host, 'section', 'report-audit-section', '');
            appendText(panelSection, 'h4', '', 'Результаты панелей');
            if (!result.panels.length) appendText(panelSection, 'p', 'report-audit-empty', 'Нет включённых панелей.');
            result.panels.forEach(panel => {
                const details = document.createElement('details'); details.className = 'report-audit-panel'; panelSection.appendChild(details);
                appendText(details, 'summary', '', `${panel.title} · ${panel.state || 'без состояния'} · ${panel.included ? 'добавлена' : 'исключена'}`);
                appendText(details, 'p', 'report-audit-panel-meta', `Ключ: ${panel.key || '—'}${panel.status ? ` · ${panel.status}` : ''}`);
                appendText(details, 'pre', '', panel.text || 'Фраза не сформирована.');
            });

            const outputSection = appendText(host, 'section', 'report-audit-section', '');
            appendText(outputSection, 'h4', '', 'Итоговое сообщение');
            const output = document.createElement('textarea');
            output.className = 'report-audit-output'; output.readOnly = true; output.value = result.output;
        ...
        const open = () => {
            ...
            const modal = document.createElement('section'); modal.className = 'modal-content report-audit-modal';
            ...
            appendText(header, 'h3', '', 'Проверка сообщения');
            ...
            const content = appendText(modal, 'div', 'report-audit-content', '');
            const actions = appendText(modal, 'div', 'modal-actions report-audit-actions', '');
            const rerun = appendText(actions, 'button', 'btn btn-primary report-audit-rerun', 'Проверить повторно'); rerun.type = 'button';
            document.body.appendChild(overlay); overlay.style.display = 'flex'; activeOverlay = overlay;
            let running = false;
            const run = async () => {
                if (running) return;
                running = true; rerun.disabled = true; content.textContent = '';
                const controller = new AbortController(); overlay.controller = controller;
                status.textContent = 'Проверяем шаблоны и получаем данные панелей…';
                try {
                    const collected = await collect(controller.signal, message => {
                        if (status.isConnected) status.textContent = message;
                    });
                    const result = audit(reportEngine, collected);
                    render(content, result);
                    status.textContent = result.summary.errors
                        ? `Проверка завершена: найдено ошибок ${result.summary.errors}.`
                        : 'Проверка завершена: сообщение формируется без ошибок.';
                } catch (error) {
            ...
            closeButton.focus(); void run();
        };
        return Object.freeze({ open, close });
    }

    root.DashBridgeReportAudit = Object.freeze({ audit, runEngineSelfCheck, create });
})(globalThis);
