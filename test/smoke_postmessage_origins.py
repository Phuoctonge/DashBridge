...
FILES = {
    "dashbridge": (ROOT / "js/pages/dashbridge.js").read_text(encoding="utf-8"),
...
checks = {
    "dashboard has one guarded postMessage wrapper": "function postToDashboardFrame(iframe, message)" in FILES["dashbridge"],
    ...
    "dashboard handles a navigation race": "iframe.contentWindow.postMessage(message, targetOrigin);" in FILES["dashbridge"]
        and "An iframe can begin navigating between the ready check and postMessage." in FILES["dashbridge"],
    "dashboard waits for an iframe ready signal": "e.data.action === 'dashbridgeIframeReady'" in FILES["dashbridge"],
